...
async function consultarDNI(dni) {
	...
	try {
		const response = await fetch(`https://habilitado.glitch.me/api/consulta/${dni}`);
		if (!response.ok) {
...
async function actualizarIdentificador(id, nuevoIdentificador) {
	...
	try {
		const response = await fetch(`https://habilitado.glitch.me/api/actualizar-identificador/${id}`, {
			method: 'PUT',
			headers: {
...
function bienvenida(nombre) {
	...
	setTimeout(() => {
		window.location.href = "registrado/index.html";
	}, 3000);
...
//memorizar numero de ingreso
// Función para establecer una cookie
function setCookieDni(name, value, days) {
    let expires = "";
    if (days) {
        const date = new Date();
